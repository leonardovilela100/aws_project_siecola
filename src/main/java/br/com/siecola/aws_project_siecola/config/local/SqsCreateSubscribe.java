...
public class SqsCreateSubscribe {
    ...
    public SqsCreateSubscribe(AmazonSNS snsCliente,
                              @Qualifier("productEventsTopic") Topic productEventsTopic) {
        AmazonSQS sqscliente =  AmazonSQSClient.builder()
         ...
         String productEventsQueueUrl = sqscliente.createQueue(
                 new CreateQueueRequest("product-events")).getQueueUrl();

        Topics.subscribeQueue(snsCliente, sqscliente, productEventsTopic.getTopicArn(), productEventsQueueUrl);
    }
}
